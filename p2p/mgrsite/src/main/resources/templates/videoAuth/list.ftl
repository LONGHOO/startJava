	...
	$(function(){
        ...
        $input.typeahead({
			source:function(username , callback){
			    $.post("/listUserByAutoComplate" , {"username":username} ,function(source){
			...
			<div class="col-sm-9">
				...
				<div class="row">
					...
					<form id="searchForm" class="form-inline" method="post" action="/videoAuth">
						...
						<div class="form-group">
							<label>预约用户</label>
							<input class="form-control" style="width: 130px;" type="text" name="username" id="username" value="${qo.username!''}" />
						</div>
						<div class="form-group">
					...
					<table class="table">
						...
						<tbody>
							...
							<tr>
								...
								<td>${(vo.remark)!''}</td>
								<td>${(vo.auditTime?string("yyyy-MM-dd  HH:mm:ss"))!""}</td>
								<td>
									<#if vo.state==0>
										<a class="audit" data-dataid="${vo.id}">审核</a>
									</#if>
								</td>
							</tr>
							</#list>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="editForm" class="form-horizontal" method="post" action="/videoAuth_audit">
