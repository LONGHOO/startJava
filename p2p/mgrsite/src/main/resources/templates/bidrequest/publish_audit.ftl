				...
				<div class="panel panel-default">
					<table class="table">
						...
						<#list pageInfo.list as data>
							<tr>
								<td>
									<a target="_blank" href="/borrow_info?id=${data.id}">${data.title}</a>&emsp;<span class="label label-primary">${data.bidRequestTypeDisplay}</span>
								</td>
			        	...
			        	<div class="form-group">
				        	...
				        	<div class="col-sm-3">
				        		<input type="text" placeholder="如果为空,立即发标" class="form-control"
									   name="publishTime" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'%y-%M-%d {%H+1}:%m:%s'})" />
				        	</div>
			        	</div>
