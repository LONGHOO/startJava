...
<script type="text/javascript">
	$(function() {
		...
		$(".auditClass").click(function(){
			...
			$("#myModal [name=rechargeDate]").html(data.rechargeDate);
		});
					...
					<table class="table">
						...
						<#list pageInfo.list as info>
							<tr>
								...
								<td>${info.rechargeDate?string('yyyy-MM-dd')}</td>
								<td>${info.amount}</td>
								<td>
									<script>
										var str="("+SITE_BANK_TYPE_NAME_MAP["${info.bankInfo.bankName}"]+")
										${info.bankInfo.accountNumber}${info.bankInfo.bankForkname}";
										document.write(str);
									</script>
								</td>
		      	...
		      	<form class="form-horizontal" id="editform" method="post" action="/rechargeOffline_audit">
					<fieldset>
			        	...
			        	<div class="form-group">
				        	...
				        	<div class="col-sm-6">
				        		<label class="form-control" name="rechargeDate"></label>
				        	</div>
			        	</div>
						<div class="form-group">
