	...
	<script type="text/javascript">
		$(function(){
			...
			$(".delete_Btn").click(function(){
				...
				$.get("/systemDictionaryItem_del?id="+data.id,function(response){
					if(response.success){
						$.messager.popup("删除成功");
						window.location.reload();
					}else{
						$.messager.popup(response.msg);
					}
				})
			})
            //分组被点击
								...
								<#list pageInfo.list as vo>
									<tr>
										...
										<td>
											...
											|
											<a href="javascript:void(-1);" data-json='${vo.jsonString}' class="delete_Btn">删除</a> &nbsp;
										</td>
									</tr>
								</#list>
								</tbody>
